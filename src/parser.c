...
layer parse_activation(list *options, size_params params)
{
    char *activation_s = option_find_str(options, "activation", "linear");
    ACTIVATION activation = get_activation(activation_s);
    int prelu_Switch_Flag = option_find_int(options, "prelu", 0);

    layer l = make_activation_layer(params.batch, params.inputs, activation, params.c, params.w, params.h, prelu_Switch_Flag);
...
void save_activation_weights(layer l, FILE *fp)
{
#ifdef GPU
    if(gpu_index >= 0){
        pull_activation_layer(l);
    }
#endif
...
void save_weights_upto(network *net, char *filename, int cutoff)
{
    ...
    for(i = 0; i < net->n && i < cutoff; ++i){
        ...
        } if((l.type == ACTIVE) && (l.prelu_Switch_Flag == 1)){
            save_activation_weights(l, fp);
        } if(l.type == RNN){
...
void load_activation_weights(layer l, FILE *fp, int transpose)
{
    //fread(l.biases, sizeof(float), l.outputs, fp);
    //printf("start to load activation weights\n");
    ...
    for(i=0; i<l.out_c; i++)
    {
    		printf("the activation value is %f\n", l.weights[i]);
    	}
    //printf("Biases: %f mean %f variance\n", mean_array(l.biases, l.outputs), variance_array(l.biases, l.outputs));
    //printf("Weights: %f mean %f variance\n", mean_array(l.weights, l.outputs*l.inputs), variance_array(l.weights, l.outputs*l.inputs));
    if (l.batch_normalize && (!l.dontloadscales)){
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    for(i = start; i < net->n && i < cutoff; ++i){
        ...
        if((l.type == ACTIVE) && (l.prelu_Switch_Flag == 1)){
            load_activation_weights(l, fp, transpose);
        }
        if(l.type == BATCHNORM){
