...
__global__ void forward_activation_layer_kernel(int n, int w, int h, int c, float *input_gpu, float *output_gpu, float *weights_gpu)
{
    int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(id >= n) return;
    ...
    for(i = 0; i < w*h; ++i){
		...
		int factor = output_gpu[out_index] >= 0 ? 1 : weights_gpu[in_index];
    		output_gpu[out_index] *= factor;
    }
}

extern "C" void forward_activation_layer_kernel_gpu(int n, int w, int h, int c, float *input_gpu, float *output_gpu, float *weights_gpu)
{	
	forward_activation_layer_kernel<<<cuda_gridsize(n), BLOCK>>>(n, w, h, c, input_gpu, output_gpu, weights_gpu);
	check_error(cudaPeekAtLastError());
}

__global__ void backward_activation_layer_kernel(int n, int w, int h, int c, float *input_gpu, float *output_gpu, float *in_delta_gpu, float *out_delta_gpu, float *weights_gpu, float *weights_updates_gpu)
{
    int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(id >= n) return;
    ...
    for(i = 0; i < w*h; ++i){
	    int out_index = i + h*w*(k + b*c);
		int back_factor = input_gpu[out_index] >= 0 ? 1 : weights_gpu[in_index];
		if(in_delta_gpu) in_delta_gpu[out_index] += out_delta_gpu[out_index] * back_factor;
		
		int weights_updates_factor = input_gpu[out_index] >= 0 ? 0 : input_gpu[out_index];
		weights_updates_gpu[in_index] +=  out_delta_gpu[out_index]*weights_updates_factor;  //is this right?
    }
}

extern "C" void backward_activation_layer_kernel_gpu(int n, int w, int h, int c, float *input_gpu, float *output_gpu, float *in_delta_gpu, float *out_delta_gpu, float *weights_gpu, float *weights_updates_gpu)
{	
	backward_activation_layer_kernel<<<cuda_gridsize(n), BLOCK>>>(n, w, h, c, input_gpu, output_gpu, in_delta_gpu, out_delta_gpu, weights_gpu, weights_updates_gpu);
	check_error(cudaPeekAtLastError());
}
